    ...
    public function connectToDatabases ( $username='Guest', $dbsConfigFile=null ) {
        ...
        if ($username!=='admin' && $username!=='Guest') {
            ...
            $ret[] = [
                'ct' => 'couchdb',
                'cRec' => $cRec,
                'conn' => $this->connectToDatabase ( $username, 'couchdb' ) // !! is_null($cRec) inside this call. meaning we use $_COOKIE['cdb_authSession_cookie]
            ];
        ...
        if (($username=='admin' || (is_object($this->findConnection('couchdb')) && is_null($this->findConnection('couchdb')->username)))) {
            ...
            if (!file_exists($domainConfigsPath.$configFilename)) $configFilename = 'databases.username-Guest.json';
             //echo '<pre style="color:lime">'; var_dump($domainConfigsPath.$configFilename); var_dump(file_exists($domainConfigsPath.$configFilename)); echo '</pre>';
            ...
            $dbt = $p['databases'];
            //echo '<pre>t74;'; var_dump ($dbt); echo '</pre>';
...
*/
    ...
    public function listUsers_debugFormat () {
        return $this->callAllDataSets ('listUsers_debugFormat');
    }

    public function listUsers_frontpageFormat () {
        return $this->callAllDataSets ('listUsers_frontpageFormat');
    }
